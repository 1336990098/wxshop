    ...
    <div id="page-infinite-scroll-bottom" class="page">
                ...
                <ul id="sortListUl" class="list">
                    ...
                    @foreach($cateInfo as $v)
                        <li sortid='100' reletype='1' linkaddr=''>
                            <span class='items list'  cate_id="{{$v->cate_id}}">
                                {{$v->cate_name}}
                            </span>
                        </li>
                    @endforeach
                                ...
                                @foreach($goodsInfo as $v)
                                    <li id="23468">
                                        <span class="gList_l fl">
                                            <a href="{{url('goods/shopcontent')}}?goods_id={{$v->goods_id}}"><img class="lazy" data-original="/goodsimg/{{$v['goods_img']}}"></a>
                                        </span>
                                        <div class="gList_r">
                                            <a href="{{url('goods/shopcontent')}}?goods_id={{$v->goods_id}}"><h3 class="gray6">{{$v['goods_name']}}</h3></a>
        ...
        <div class="footer clearfix" >
            <ul>
                ...
                <li class="f_announced"><a href="{{url('goods/goodslist')}}" class="hover"><i></i>全部商品</a></li>
...
<script>
    jQuery(document).ready(function() {
        $("img.lazy").lazyload({
            placeholder : "/goodsimg/{{$v['goods_img']}}",
            effect: "fadeIn",
        });
    });
</script>
...
<script>
    ...
    $('#sortListUl li').click(function(){
        var _this=$(this);
        var id=_this.children().attr('cate_id');
        $.ajax({
            type:"post",
